...
int MonitorThread()
{
    ...
    //while (1) 
    {
        ...
        {
            axisId=i;
            ret=adx_ReadLogicalPos(axisId,index,&lgcPos[i]);
            if(ret!=0)
            {
                log_info("adx_ReadLogicalPos error ret=%d\n",ret);
            }
            Sleep(5);
            ret=adx_ReadActualPos(axisId,index,&actPos[i]);
            if(ret!=0)
            {
                log_info("adx_ReadLogicalPos error ret=%d\n",ret);
            }
            Sleep(5);
            ret=adx_ReadStatus(axisId, index, &state[i]);
            if(ret!=0)
            {
...
int setRigisterFun()
{
    ...
    for(int i=0;i<4;i++)
    {
        axisId=i;
        //设置电机使能函数
        ret=adx_RegisterSetMotorEnableFun(i, 0,setMotorEnableSim);
        if(ret!=0)
        {
            printf("adx_RegisterSetMotorEnableFun error ret=%d\n",ret);
            return ret;
        }
        //设置电机参考位置函数
        ret=adx_RegisterSetMotorRefPosFun(i, 0,setMotorPositionSim);
        if(ret!=0)
        {
            printf("adx_RegisterSetMotorRefPosFun error ret=%d\n",ret);
            return ret;
        }
        //获取电机实际位置函数
        ret=adx_RegisterGetMotorActPosFun(i, 0,getMotorPositionSim);
        if(ret!=0)
        {
            printf("adx_RegisterGetMotorActPosFun error ret=%d\n",ret);
            return ret;
        }
    }
    return 0;
...
int main(void)
{
   ...
   // 初始化核心;
    ...
    if(ret)
    {
        printf("setRigisterFun error ret=%d\n",ret);
        return 1;
    }
   //运动控制主循环线程
