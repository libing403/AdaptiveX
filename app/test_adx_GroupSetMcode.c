...
int main()
{
    ...
    ret=adx_GroupWriteParameter(grpId, index, mcSmoothLevel, 50, 0);
    if (ret)
    {
    ...
    for(int j=0;j<pathLength;j++)
    {
        ...
        if(j%2==0)//偶数的点设置M代码
        {
            ret=adx_GroupSetMcode(grpId,index,&forwardValue,&reverseValue,setValue,setValue,1);
            if (ret)
            {
                printf("adx_GroupSetMcode error %d\n", ret);
                return -1;
            }
    ...
    while(1) {
        ...
        //printf("forwardValue=%d,reverseValue=%d\n",forwardValue,reverseValue);       
    }
    adx_CoreClose();//关闭运动库
    return ret;
}
